...
for _, row in top5.iterrows():
    values = [row[nut] for nut in nutrients_to_plot]
    fig.add_trace(go.Scatterpolar(
        r=values + [values[0]],  # Close the loop
        theta=nutrients_to_plot + [nutrients_to_plot[0]],
        fill='toself',
        name=row["name"]
    ))
...
fig.update_layout(
    polar=dict(
        ...
        angularaxis=dict(rotation=90),  # optional: rotates labels to top
        domain=dict(x=[0, 1], y=[0, 1])  # expands plot to full area
    ),
    showlegend=True,
    height=600,
    ...
    legend=dict(
        ...
        itemwidth=30,        # max width before wrapping
    )
)
